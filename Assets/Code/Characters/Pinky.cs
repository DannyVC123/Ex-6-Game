...
public class Pinky : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        currMode = SCATTER;
        cornerTile = new Vector2Int(1, height - 2);
    ...
    private void CalculateTargetTile()
    {
        ...
        targetTile += directions[pacman.GetCurrOrientation()] * targetOffset;

        float stepSize = 1f / targetOffset;
        if (!inBounds(targetTile) || tiles[targetTile.x, targetTile.y] == WALL)
        {
            for (int i = targetOffset - 1; i >= 0; i--)
            {
                Vector2Int potentialTarget = Vector2Int.RoundToInt(Vector2.Lerp(pacmanPos, targetTile, i * stepSize));
